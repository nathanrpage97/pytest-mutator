...
from pony.orm import RowNotFound
from pony.orm import Set
from pony.orm import db_session
from pony.orm import select

from . import BAD_SURVIVED
from . import BAD_TIMEOUT
from . import OK_KILLED
from . import OK_SUSPICIOUS
from . import UNTESTED
from . import Context
from . import RelativeMutationID
from . import mutate
...
def print_result_cache_junitxml(dict_synonyms, suspicious_policy, untested_policy):
    ...
    for filename, mutants in groupby(mutant_list, key=lambda x: x.line.sourcefile.filename):
        for mutant in mutants:
            tc = TestCase(
                "Mutant #{}".format(mutant.id), file=filename, line=mutant.line.line_number, stdout=mutant.line.line
            )
            ...
            if mutant.status == BAD_TIMEOUT:
                tc.add_error_info(
                    message=mutant.status, error_type="timeout", output=get_unified_diff(mutant.id, dict_synonyms)
                )
    ...
    with open("html/index.html", "w") as index_file:
        ...
        for filename, mutants in groupby(mutants, key=lambda x: x.line.sourcefile.filename):
            ...
            with open(join(report_filename + ".html"), "w") as f:
                ...
                index_file.write(
                    '<tr><td><a href="%s.html">%s</a></td><td>%s</td><td>%s</td><td>%.2f</td><td>%s</td>'
                    % (
                        filename,
                        filename,
                        len(mutants),
