...
class RelativeMutationID(object):
    ...
    def __repr__(self):
        return 'MutationID(line="{}", index={}, line_number={}, filename={})'.format(
            self.line, self.index, self.line_number, self.filename
        )
...
class Context(object):
    ...
    def exclude_line(self):
        return self.current_line_index in self.pragma_no_mutate_lines or should_exclude(
            context=self, config=self.config
        )
    ...
    def mutation_id_of_current_index(self):
        return RelativeMutationID(
            filename=self.filename, line=self.current_source_line, index=self.index, line_number=self.current_line_index
        )
...
def queue_mutants(*, progress, config, mutants_queue, mutations_by_file):
    from .cache import get_cached_mutation_statuses
...
def run_mutation(context: Context, callback) -> str:
    ...
    """
    from .cache import cached_mutation_status
...
def popen_streaming_output(cmd, callback, timeout=None):
    ...
    if os.name == "nt":  # pragma: no cover
        ...
        process = subprocess.Popen(shlex.split(cmd, posix=True), stdout=slave, stderr=slave)
...
def run_mutation_tests(config, progress, mutations_by_file):
    ...
    """
    from .cache import update_mutant_status
    ...
    while t.is_alive():
        ...
        if command == "end":
            ...
            update_mutant_status(
                file_to_mutate=filename, mutation_id=mutation_id, status=status, tests_hash=config.hash_of_tests
            )
...
def read_patch_data(patch_file_path):
    ...
    return {diff.header.new_path: {change.new for change in diff.changes if change.old is None} for diff in diffs}
...
def add_mutations_by_file(mutations_by_file, filename, dict_synonyms, config):
    ...
    try:
        ...
        from .cache import register_mutants
